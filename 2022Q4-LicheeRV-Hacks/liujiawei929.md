...
* 测试一下当前已有的linux发行版在riscv架构上的运行效果
...
比较关注riscv架构，得知国内外的linux发行版都在做适配。
...
debain、openEuler、Fedora、ubuntu等（尽力而为吧）的使用体验。
